...
def segment_reps(smoothed_data, exercise):
    """
    ...
    """
    reference_columns = {
        "Side-Lateral-Raise" : "LEFT_ELBOW_y",
        "Lunge" : "LEFT_HIP_y"
    }
    
    reference_column = reference_columns[exercise]
...
def draw_equal_scaled_skeleton(keypoints1, keypoints2, width, height):
    ...
    for kp1, kp2 in mp_pose.POSE_CONNECTIONS:
        ...
        cv2.line(canvas, tuple(keypoints1[kp1, :2].astype(int)), tuple(keypoints1[kp2, :2].astype(int)), (0, 255, 0), 5, lineType=cv2.LINE_AA)
        cv2.circle(canvas, tuple(keypoints1[kp1, :2].astype(int)), 10, (0, 255, 0), -1)
        cv2.circle(canvas, tuple(keypoints1[kp2, :2].astype(int)), 10, (0, 255, 0), -1)
        
        # 두 번째 스켈레톤 (초록색)
        cv2.line(canvas, tuple(keypoints2[kp1, :2].astype(int)), tuple(keypoints2[kp2, :2].astype(int)), (0, 0, 225), 5, lineType=cv2.LINE_AA)
        cv2.circle(canvas, tuple(keypoints2[kp1, :2].astype(int)), 10, (0, 0, 255), -1)
        cv2.circle(canvas, tuple(keypoints2[kp2, :2].astype(int)), 10, (0, 0, 255), -1)
